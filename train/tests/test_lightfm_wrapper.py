...
def test_no_side_features(inputs,expected_predictions):
    ...
    lfm.run(train_df=pd.DataFrame({'users':['user1','user2'],'items':['item1','item2']}),
                         ...
                         [
                             ({
                               'postprocess_kwargs':{
                                   'remove_train_interactions_from_test':False,
                                   'fill_most_popular_items':False}},
                              {'train':{'user1':['item1','item2','item3'], 
                                        'user2':['item1','item2','item3']},
                               'test':{'user1':['item1','item2','item3'], 
                                       'user3':[]}}),
                             ({'postprocess_kwargs':{
                                   'remove_train_interactions_from_test':True,
                                   'fill_most_popular_items':False}},
...
def test_no_user_side_features(inputs,expected_predictions):
    lfm = lightfm_wrapper.LightFM(model_kwargs={
                                  'no_components':10,
                                  'learning_rate':0.05,
                                  'loss':'warp',
    ...
    lfm.run(train_df=pd.DataFrame({'users':['user1','user2'],'items':['item1','item2']}),
            test_df=pd.DataFrame({'users':['user1','user3'],'items':['item1','item3']}),  
            train_user_features_dict={}, train_item_features_dict={'item1':{'type':'shirt'}, 'item2':{'type':'dress'}},
            test_user_features_dict={}, test_item_features_dict={'item3':{'type':'pants'}})
    lfm.predictions_dict = {k:{kk:sorted(vv) for kk,vv in v.items()} for k,v in lfm.predictions_dict.items()}
    assert lfm.predictions_dict == expected_predictions
...
def test_no_item_side_features(inputs,expected_predictions):
    lfm = lightfm_wrapper.LightFM(model_kwargs={
                                  'no_components':10,
                                  'learning_rate':0.05,
                                  'loss':'warp',
    ...
    lfm.run(train_df=pd.DataFrame({'users':['user1','user2'],'items':['item1','item2']}),
            test_df=pd.DataFrame({'users':['user1','user3'],'items':['item1','item3']}), 
            train_user_features_dict={'user1':{'gender':'m'}, 'user2':{'gender':'f'}}, train_item_features_dict={},
            test_user_features_dict={'user3':{'gender':'m'}}, test_item_features_dict={})
    lfm.predictions_dict = {k:{kk:sorted(vv) for kk,vv in v.items()} for k,v in lfm.predictions_dict.items()}
    assert lfm.predictions_dict == expected_predictions
...
def test_full_side_features_small(inputs,expected_predictions):
    lfm = lightfm_wrapper.LightFM(model_kwargs={
                                  'no_components':10,
                                  'learning_rate':0.05,
                                  'loss':'warp',
...
def test_full_side_features_large(inputs,expected_predictions):
    lfm = lightfm_wrapper.LightFM(model_kwargs={
                                  'no_components':10,
                                  'learning_rate':0.05,
                                  'loss':'warp',
                                  ...
                                  train_kwargs={
                                   ...
                                   'plot':False},        
                                  postprocess_kwargs=inputs['postprocess_kwargs'])
    lfm.run(train_df=pd.DataFrame({'users':['user1','user2','user1'],'items':['item1','item2','item2']}),
            ...
            train_user_features_dict={'user1':{'gender':'m','favorite_products':['shirt','pants'], 'embedding':np.array([1,1,1])}, 
                                      'user2':{'gender':'f', 'favorite_products':['shirt','pants', 'sweater'], 
                                               'embedding':np.array([2,2,2])}}, 
